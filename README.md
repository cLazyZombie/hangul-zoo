...
2. 동물 그림 아래 빈칸에, 하단의 타일을 드래그해서 넣습니다.
   - 처음 10번은 초성모드입니다. 초성은 고정으로 보여 주고, 중성/종성 조각을 끼워 넣습니다.
   - 10번 맞춘 뒤에는 글자 타일 모드로 바뀌며, 5초간 못 맞추면 아직 못 맞춘 첫 글자의 초성 힌트가 나타납니다.
   - 20번 맞춘 뒤에는 힌트 없이 글자 타일을 찾습니다.
...
- Edge TTS로 미리 생성한 한국어 MP3 — 동물 이름, 글자/자모 타일, 칭찬 문구 재생
...
Edge TTS의 한국어 여성 음성(`ko-KR-SunHiNeural`)으로 `audio/`의 MP3를 다시 생성할 수 있습니다. 동물 이름, 글자 타일용 음절, 자모 조각 발음, 칭찬 문구가 함께 생성됩니다. 실행 시 인터넷 연결과 `edge-tts` 패키지가 필요합니다.
