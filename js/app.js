...
const FALLBACK_JAMO = ['ㅏ', 'ㅓ', 'ㅗ', 'ㅜ', 'ㅡ', 'ㅣ', 'ㅐ', 'ㅔ', 'ㅑ', 'ㅕ', 'ㅛ', 'ㅠ', 'ㄱ', 'ㄴ', 'ㄹ', 'ㅁ', 'ㅅ', 'ㅇ'];

const JAMO_MODE_UNTIL_CORRECT = 10;  // 이만큼 맞추기 전까지는 초성모드
const HINT_MODE_UNTIL_CORRECT = 20;  // 이만큼 맞추기 전까지는 초성 힌트 단계
...
const CONSONANT_SOUNDS = {
  ...
  'ㄻ': '리을 미음', 'ㄼ': '리을 비읍', 'ㄽ': '리을 시옷', 'ㄾ': '리을 티읕',
  'ㄿ': '리을 피읖', 'ㅀ': '리을 히읗', 'ㅄ': '비읍 시옷',
};
const VOWEL_SOUNDS = {
...
const SYMBOL_SOUNDS = { ...CONSONANT_SOUNDS, ...VOWEL_SOUNDS };

let animals = [];
let progress = loadProgress();  // { [이름]: { correct, wrong } }
let current = null;             // { animal, chars, stage, filledCount, targetCount }
let lastName = null;
let koVoice = null;
...
function buildJamoDistractors(answerJamo, count) {
  const answerSet = new Set(answerJamo);
  const pool = new Set();
  for (const a of animals) {
    if (current && a.name === current.animal.name) continue;
    for (const ch of Array.from(a.name)) {
      jamoPiecesOf(ch).forEach((piece) => {
        if (!answerSet.has(piece)) pool.add(piece);
      });
    }
  }
  for (const piece of FALLBACK_JAMO) {
    if (!answerSet.has(piece)) pool.add(piece);
  }
  return shuffle(Array.from(pool)).slice(0, count);
}
...
function jamoPiecesOf(ch) {
  const parts = decomposeSyllable(ch);
  if (!parts) return [ch];
  return parts.final ? [parts.medial, parts.final] : [parts.medial];
}

function stageForAnimal(animal) {
  const correct = statsOf(animal.name).correct;
  if (correct < JAMO_MODE_UNTIL_CORRECT) return 'jamo';
  if (correct < HINT_MODE_UNTIL_CORRECT) return 'syllable-hint';
  return 'syllable';
}

function speechTextForSymbol(symbol) {
  return SYMBOL_SOUNDS[symbol] || symbol;
}

function speakTileSound(symbol) {
  speak(speechTextForSymbol(symbol));
}

function buildJamoModel(chars) {
  let slotIndex = 0;
  const slots = [];
  const syllables = chars.map((ch, syllableIndex) => {
    const parts = decomposeSyllable(ch);
    if (!parts) {
      const slot = { id: `jamo-${slotIndex}`, char: ch, type: 'medial', syllableIndex, index: slotIndex };
      slotIndex += 1;
      slots.push(slot);
      return { char: ch, initial: '', medialSlot: slot, finalSlot: null };
    }
    ...
    return { char: ch, initial: parts.initial, medialSlot, finalSlot };
  });
  return { syllables, slots };
}
...
function scheduleHint() {
  ...
  if (!current) return;
  if (current.stage !== 'syllable-hint') return;
...
function createBlank(slot, extraClass = '') {
  const blank = document.createElement('div');
  blank.className = extraClass ? `blank ${extraClass}` : 'blank';
  blank.dataset.char = slot.char;
  blank.dataset.index = String(slot.index);
  if (slot.id) blank.dataset.slotId = slot.id;
  if (slot.type) blank.dataset.type = slot.type;
  return blank;
}

function renderSyllableBlanks(blanksEl) {
  blanksEl.className = 'blanks';
  current.chars.forEach((ch, i) => {
    blanksEl.appendChild(createBlank({ char: ch, index: i }));
  });
}

function renderJamoBlanks(blanksEl) {
  blanksEl.className = 'blanks jamo-blanks';
  current.jamoSyllables.forEach((syllable, i) => {
    const group = document.createElement('div');
    group.className = 'jamo-syllable';

    const top = document.createElement('div');
    top.className = 'jamo-top';

    const initial = document.createElement('button');
    initial.className = `jamo-initial c${i % TILE_COLORS}`;
    initial.type = 'button';
    initial.textContent = syllable.initial || syllable.char;
    initial.dataset.char = syllable.initial || syllable.char;
    initial.setAttribute('aria-label', `${initial.dataset.char} 듣기`);
    initial.addEventListener('click', (e) => {
      e.stopPropagation();
      speakTileSound(initial.dataset.char);
    });

    top.appendChild(initial);
    top.appendChild(createBlank(syllable.medialSlot, 'jamo-medial'));
    group.appendChild(top);

    if (syllable.finalSlot) {
      group.appendChild(createBlank(syllable.finalSlot, 'jamo-final'));
    }

    blanksEl.appendChild(group);
  });
}

function renderTileTray(tileChars) {
...
function startQuestion(animal) {
  const chars = Array.from(animal.name);
  const stage = stageForAnimal(animal);
  const jamoModel = stage === 'jamo' ? buildJamoModel(chars) : null;
  current = {
    animal,
    chars,
    stage,
    filledCount: 0,
    targetCount: jamoModel ? jamoModel.slots.length : chars.length,
    jamoSyllables: jamoModel ? jamoModel.syllables : [],
    jamoSlots: jamoModel ? jamoModel.slots : [],
  };
  ...
  blanksEl.innerHTML = '';
  if (current.stage === 'jamo') {
    renderJamoBlanks(blanksEl);
  } else {
    renderSyllableBlanks(blanksEl);
    scheduleHint();
  }

  const answerTiles = current.stage === 'jamo'
    ? current.jamoSlots.map((slot) => slot.char)
    : current.chars;
  const distractorCount = Math.max(3, TILE_TOTAL - answerTiles.length);
  const distractors = current.stage === 'jamo'
    ? buildJamoDistractors(answerTiles, distractorCount)
    : buildDistractors(answerTiles, distractorCount);
  renderTileTray(shuffle([...answerTiles, ...distractors]));
...
function attachTileEvents(tile) {
  ...
  tile.addEventListener('click', () => speakTileSound(tile.dataset.char));
...
function onTouchEnd(e) {
  ...
  if (touchTile) {
    ...
    if (blank) {
      ...
      speakTileSound(touchTile.dataset.char);
    }
    touchTile = null;
    touchStart = null;
  }
...
function judgeDrop(tile, blank) {
  ...
  if (tile.dataset.char === blank.dataset.char) {
    ...
    speakTileSound(blank.dataset.char);

    current.filledCount += 1;
    if (current.filledCount === current.targetCount) {
      clearTimeout(hintTimer);
